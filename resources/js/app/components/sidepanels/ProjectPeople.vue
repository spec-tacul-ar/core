...
export default {
    ...
    computed: {
        ...
        can_leave() {
            const me = this.project.collaborations.firstWhere('account_id', useAuthStore().account.id);

            if (me.role === 'owner') {
                return this.project.collaborations.where('role', 'owner').count() > 1;
            }
        ...
        collaborations() {
            ...
            return this.project.collaborations
                .sortBy('created_at')
                .sortBy(collaboration => order[collaboration.role.value]);
        },
        invitations() {
    ...
    methods: {
        leave() {
            ...
            this.api.post('collaborations/' + me.id + '/delete')
                .then(() => {
        ...
        submit() {
            ...
            this.api.post('invitations', {...this.form, 'project_id': this.project_id})
                .then((result) => {
