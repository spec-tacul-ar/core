...
export default {
    ...
    computed: {
        ...
        editing_channel() {
            return this.feature_id ? 'features.editing.' + this.feature_id : null;
        },
    },
    data() {
        return {
    ...
    methods: {
        submit() {
            ...
            const endpoint = this.is_creating ? 'features' : 'features/' + this.feature_id + '/edit';
    ...
    async setup(props) {
        ...
        if (!feature) {
            const result = await api.get('projects/' + props.project_id, {query: {'hydrate': true}});
            Feature.repository('projects').save({...result.data, is_hydrated: true});
            feature = repository.find(props.feature_id);
        }
